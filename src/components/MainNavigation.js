...
const langues = [{ code: 'en', name: 'English', country_code: 'en' }, { code: 'hu', name: 'Magyar', country_code: 'hu' }];


const MainNavigation = () => {
    const { t } = useTranslation();

    const navigationElements = t('navigation', { returnObjects: true }).map((navs) => (navs));

    return (
        <header className={classes.header}>
            <nav>
                <ul className={classes.list}>
                    {navigationElements.map((navs) =>
                        <li key={navs.id}>
                            <NavLink
                    ...
                    <li className={classes.languageSVG}>{languageSVG}
                        <ul className={classes.dropdown}>
                            {langues.map(({ code, name, country_code }) => (
                                <li key={country_code}
